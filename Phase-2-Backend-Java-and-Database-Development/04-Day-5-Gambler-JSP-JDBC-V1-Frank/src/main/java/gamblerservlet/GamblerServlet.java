...
public class GamblerServlet extends HttpServlet {
	...
	public GamblerServlet() throws IOException, SQLException {
		super();
		// In prior JSP examples, we were accesses a memory data source
		// Using the JdbcDao to access the mySql database
		// No other code changes in this program
		// (THIS is why we use interface for the DAO)
		// Code in this program does not depend on where the data is stored
		//      As long as the DAO used is a GamblerDao (interface) this code works
//		gamblerTable = new MemoryGamblerDao;  // Used in prior versions of the program
		gamblerTable = new JdbcGamblerDao();
...
// We are ready to build the HTML to display each Gambler in the List of Gamblers
	...
	private void performUpdate(HttpServletRequest request, long id2Update) throws DataBaseUpdateException, DataBaseDeleteException {
		...
		*/
		...
		} catch (DataBaseUpdateException | DataBaseDeleteException | SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
